...
namespace heatBurners {

    namespace {
        std::list<HeatBurner*> activeParticles_;
    }

    HeatBurner::HeatBurner(Vector2f const& location, Vector2f const& direction, Vector2f const& velocity):
               Particle(spaceObjects::oHeatBurner, location, 1.f, 0.f, sf::Randomizer::Random(0.4f, 0.5f)) {
    ...
    void spawn(Vector2f const& location, Vector2f const& direction, Vector2f const& sourceVelocity) {
        activeParticles_.push_back(new HeatBurner(location, direction, sourceVelocity));
    }

    void draw() {
        for (std::list<HeatBurner*>::iterator it = activeParticles_.begin(); it != activeParticles_.end(); ++it)
            (*it)->draw();
    }

    void update() {
        std::list<HeatBurner*>::iterator it = activeParticles_.begin();
