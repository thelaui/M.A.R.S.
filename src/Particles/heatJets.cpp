...
namespace heatJets {
    ...
    namespace {
        std::list<HeatJet*> activeParticles_;
    }

    HeatJet::HeatJet(Vector2f const& location, Vector2f const& direction, Vector2f const& velocity):
               Particle(spaceObjects::oHeatJet, location, 1.f, 0.f, sf::Randomizer::Random(0.5f, 1.0f)) {
    ...
    void spawn(Vector2f const& location, Vector2f const& direction, Vector2f const& sourceVelocity) {
        if (settings::C_shaders)
            activeParticles_.push_back(new HeatJet(location, direction, sourceVelocity));
    }

    void draw() {
        for (std::list<HeatJet*>::iterator it = activeParticles_.begin(); it != activeParticles_.end(); ++it)
            (*it)->draw();
    }

    void update() {
        std::list<HeatJet*>::iterator it = activeParticles_.begin();
