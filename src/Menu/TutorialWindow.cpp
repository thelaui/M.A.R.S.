...
UiWindow* TutorialWindow::get(sf::String* title, sf::String* text, int index, bool info, bool next) {
    ...
    if (instance_ == NULL) {
        instance_ = new TutorialWindow(400, 200);
		  instance_->addWidget(new Button(next ? _("Next") : _("OK"), NULL, &kOk_, Vector2f(300,170), 90, 20));
        instance_->addWidget(new Label(title, TEXT_ALIGN_LEFT, Vector2f(10,10), 20.f, Color3f(1.f, 0.5f, 0.9f), false));
        if (info) {
				instance_->addWidget(new Label(_("Info"), TEXT_ALIGN_RIGHT, Vector2f(380,18), 12.f, Color3f(1.f, 0.5f, 0.9f), false));
        }
        else {
